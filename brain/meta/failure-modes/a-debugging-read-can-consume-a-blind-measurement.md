...
description: A polling probe that scraped the result card would have revealed the verdict for every queued case before the human labeller opened one, and the labels would still have been recorded as blind.
...
not:
  - term: "scrape the whole result card to check whether a run finished"
    why: "the card carries the verdict, so polling reveals the answer the human is supposed to label independently, and a revealed label is unrecoverable without regenerating the case"
...
Two ways it nearly went wrong in k1, both while automating case generation rather
than while labelling.

A polling probe written to answer "has this run finished" captured the result card.
The card contains the verdict chip. Running it across the queue would have revealed
the answer for every constructed case before the labeller opened the first one, and
the labels would still have been recorded as blind, because nothing in the system
can tell that an agent read the page.

Separately, localhost is unusable for blind labelling regardless of care taken in
the UI: the development RSC flight stream ships the verdict in an orphan chunk, so
the value sits in the page source even when no component renders it.

# Why it matters

Blinding failures are silent and they do not reverse. `capturedBlind` exists to
mark a label taken with the verdict on screen, and such labels are kept while being
excluded from the gate set. That control only covers reveals the *application*
performs. A probe reading the DOM out of band, or a developer reading the database
to debug, leaves the flag saying `true`.

So the debugging path needs its own reader that is structurally incapable of
revealing the measured value, and the polling path needs to request the narrowest
field that answers its question. Both are cheap to build before labelling starts
and impossible to retrofit after.
