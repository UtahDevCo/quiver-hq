...
description: Six copies of one filter existed; the three in audit and verification scripts were wrong and the three in paths with users were right. Users exercise runtime code, so drift accumulates where nothing does.
kind: failure-mode
proposed_layer: meta
...
not:
  - term: "auditing the runtime copies of a rule and assuming the monitoring copies match"
    ...
    instead: "when consolidating, check the monitoring and audit copies first — that is where the wrong ones are"
...
suspect and the tooling as incidental. It is the reverse. Runtime copies are under
...
restored after being comped keeps `error` and moves off `SKIPPED`, and it was not paid
for by the counter. All 755 matching documents are currently `SKIPPED` with no restore,
so the two conditions are equivalent today and will not stay that way.
