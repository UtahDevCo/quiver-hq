...
matched 0 files, and nothing documents what an adjusted rule would mean or when to
author one. Read it as `TAXABLE`.
...
covers this shape: an option nobody selects should leave the toolchain rather than sit
in the enum accumulating speculation. Either delete `ADJUSTED` and let the type system
reject it, or record the case it was reserved for so the next reader stops guessing.
...
Not yet attached. The sweep is a one-line grep over the rule CSVs, so an attester is
cheap; the reason it stays manual for now is that a first ADJUSTED row appearing in
data is the event that should retire this concept rather than fail a build.
