...
sources:
  - id: thread
    resource: "Drive — Invoicing proof to pull from, screenshots 14nnYyA8EP5zTVlbgaP2_jlVPQd3K_g35 / 10yAYnMJ1H10YxmJ6euPqJSDp22CGUZ_8"
    title: "brokerage claims $1,996.50; the property's leasing professional replies \"the commission amount it'll be $1,782.59\""
  - id: doa
    resource: "Delegation of Authority and Initial Underwriting Policy §4, §7"
    title: "the amount, payor, and payment conditions must be reasonably verifiable; verification should use contact information obtained independently of the applicant"
---

# The rule

The claimed figure and the acknowledged figure are **two columns**, plus a
`commissionAmountSource` discriminator (`broker_claim` | `payor_acknowledgment` |
`independent_verification`). Underwriting reads the acknowledged one, and a
transaction carrying only a claim is not underwritable.

The provenance of the *contact* is a separate column from the provenance of the
*amount*, and is load-bearing here too. The policy wants verification using contact
details obtained independently of the applicant, while the contract forbids the
purchaser from contacting the property manager at all. In the pilot the
acknowledgment therefore arrives broker-forwarded, which is applicant-supplied, and
must route to human review rather than auto-approve until counsel resolves it.
...
was written most recently. A purchase price computed off the claim is a price the
payor will not fund.

Recording both also gives the thing this business needs to sell later: per-brokerage
evidence of how accurate their claims are.
...
the commission amount it'll be **$1,782.59**." An 11% disagreement, in the first
example, from the party who actually pays.
