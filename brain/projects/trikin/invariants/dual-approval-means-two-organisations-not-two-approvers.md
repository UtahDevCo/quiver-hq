...
sources:
  - id: doa
    resource: "Delegation of Authority and Initial Underwriting Policy §2, §3, §8"
    title: "Authorized Representatives table with designee and backup per Member; per-officer purchase authority; separate authorized payment release"
  - id: founders
    resource: "Founders' Principles and Agreed Business Terms §3-4"
    title: "SBI 50% / Trikin Holdings 50%; material decisions unanimous"
  - id: approles
    resource: projects/trikin/web/src/db/schema.ts:34-56
    title: "users.appRoles — a mutable JSON array with no history, the wrong home for this"
---

# The rule

A purchase above $10,000 requires prior written approval from **both Members** of a
50/50 joint venture. Four things follow that a role array structurally cannot
express.
...
**Effective dating.** An approval made in March must be judged against the authority
in effect in March. `appRoles` is destructively mutated, so revoking an approver
today rewrites the validity of every past approval, in whichever direction happens
to be convenient.
...
each identity chains back to the approval that granted it. Toggling a role in an
admin drawer leaves no such record.
...
**decide**. An operations user can be an admin with zero authority identities: sees
every queue, can approve nothing.
...
**disjoint** from the purchase approvers. That last one is what "separate authorized
payment release" means.
...
fails when the two approvals happen to come from the same side, which is the normal
case when one Member is more responsive than the other. The control degrades exactly
...
The generalisable shape: when a rule is about *whose* approval, a role array is the
wrong primitive. It models capability, and the rule is about identity, organisation,
and time.

# Evidence

DoA §2 tabulates an "Initial Designee" and a "Backup / Replacement" per Member (Andy
Swartz for SBI, Joshua Levy for LevyCo), which is an effective-dated appointment with
a named successor. A `string[]` on a user row cannot hold that.
