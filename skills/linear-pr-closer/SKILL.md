...
4. Upload screenshot evidence when available.
   - If the session has browser DevTools access and screenshot files exist in `temp/linear/` for this ticket, attach them to the PR so reviewers can see visual evidence without checking out the branch.
   - **Use native `gh --attach`** (GitHub CLI ≥ v2.99.0) — it uploads media directly using `gh` CLI auth, no extension and no browser cookie required. Do not use the old `gh-attach` extension (`sudosubin/gh-attach`, `atani/gh-attach`) or `github.com/user-attachments/...` URL juggling.
   - **Upload workflow**:
     ...
     2. Attach the screenshots directly when posting the PR comment (the `--attach` flag is repeatable; append `#alt text` for alt text):
        ...
        **Before (bug reproduced):**
        ![before](temp/linear/<date>/<ISSUE-ID>-before.png)

        **After (fix confirmed):**
        ![after](temp/linear/<date>/<ISSUE-ID>-after.png)" \
          --attach temp/linear/<date>/<ISSUE-ID>-before.png \
          --attach temp/linear/<date>/<ISSUE-ID>-after.png
        ```
        The local `![alt](…)` paths in the body are rewritten in place to the uploaded assets.
   - If `gh --version` is below 2.99.0, the `--attach` flag is silently absent — bump `gh` before relying on it.
