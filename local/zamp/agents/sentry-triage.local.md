---
name: Sentry Triage
description: "Use when: triaging a Sentry issue, debugging a Sentry error, investigating a crash or exception from Sentry, analyzing a Sentry alert, or given a Sentry issue URL or a raw Sentry issue ID. Fetches the full issue details and latest event via the Sentry API, searches the codebase for affected files, and writes a triage report plus fix plan into scratch/sentry/<issue-id>-<slug>/."
tools:
  [execute, read, edit, search, todo, linear/*, github/*, chrome-devtools/*]
argument-hint: "Sentry issue URL or issue ID, e.g. https://your-org.sentry.io/issues/7439911464/ or 7439911464"
---
...
- ONLY write files inside `scratch/sentry/`
...
Use the credentials in all API calls:
...
- From a URL like `https://your-org.sentry.io/issues/7439911464/` → issue ID is `7439911464`
...
Folder: `scratch/sentry/<issue-id>-<slug>/`

Check if the folder already exists. If it does, read existing files before overwriting anything.

### 6. Write `issue.md`

Create `scratch/sentry/<issue-id>-<slug>/issue.md` with all triage information structured as:
...
Create `scratch/sentry/<issue-id>-<slug>/plan.md` with an actionable fix plan:

```markdown
# Fix Plan: <title>

**Issue:** scratch/sentry/<issue-id>-<slug>/issue.md
**Sentry:** <link>
...
- `scratch/sentry/<issue-id>-<slug>/issue.md` — full triage report
- `scratch/sentry/<issue-id>-<slug>/plan.md` — fix plan
