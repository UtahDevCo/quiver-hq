...
argument-hint: "Plan file path or Linear issue ID, for example: scratch/linear/2026-04-22/ABC-123-plan.md"
---
...
3. Implement the ticket.
   ...
   - If a database migration is needed, create it with `pnpm db:push` rather than writing the file manually.

4. Validate the implementation.
   - Run focused tests first, then broader checks as justified by the change.
   - Use `pnpm type-check` for type-checking.
   - Run all necessary tests; the work is not done until all relevant tests pass.
   - Run `pnpm build` when the change affects build-time behavior, routing, data loading, or production-only paths.
   - Always run an ESLint + Prettier auto-fix pass before handing off for a PR. Scope it to the affected packages when possible (e.g. `pnpm -F {package} lint --fix` and `pnpm -F {package} format`); otherwise run `pnpm lint --fix` and `pnpm format` across the monorepo. Re-run the relevant checks afterward and confirm the working tree only contains intended changes.
