...
- **A file's primary export should match its filename, kebab→Pascal — but consistency with the directory's established convention wins.** Default to the literal match (`combobox-shell.tsx` exporting `FilterBarComboboxShell` was a finding). The exception: when every sibling in a compound-component directory already carries a shared prefix, keep the prefix and let the filename drop it — `filter-bar/` has `item.tsx`→`FilterBarItem`, `list.tsx`→`FilterBarList`, `trigger.tsx`→`FilterBarTrigger`, so `single-select-item.tsx`→`FilterBarSingleSelectItem` is correct and renaming it to `SingleSelectItem` would make the new file the odd one out. Check the siblings before renaming, and say which rule you applied so the reviewer can weigh in. (James Walsh asked for the literal match on OUT-997; Chris chose sibling consistency.)
- **Don't fix a pre-existing violation inside a feature PR.** If a file already broke a rule on `main` (`filter-bar/item.tsx` was 393 lines before OUT-997 touched it), a size/structure flag against it is *pre-existing* — extracting its siblings, hoisting shared types into a `types.ts`, and renaming neighbours to satisfy it is scope creep that buries the feature in churn. Run `git show main:<path> | wc -l` before accepting a "split this file" flag on a file you only added to. Put the new variant in its own new file, leave the neighbours alone, and file a follow-up for the split. (Chris, OUT-997.)
...
**This applies to code you write, not just code you review.** Chris has had to ask for a comment cull on work authored in-session more than once. Before handing off a diff, re-read every comment you added and delete any that isn't a non-obvious _why_. A new component does not automatically earn a docblock; a new prop does not automatically earn a prop doc. Budget roughly one short comment per genuine gotcha — if a small new file has three or more, that's a signal to cut. (Chris, OUT-997.)
...
- **The DS `Button` `loading` prop *appends* a spinner (renders `{children}` + `<Spinner />`), widening the button when it enters the loading state.** Fine for most buttons, but when one needs a *no-resize-jank* loading state, a hand-rolled overlay is correct — keep the label as the width-defining element (hidden via `invisible`) and absolutely-center the spinner over it so width stays constant across idle/dirty/loading. CodeRabbit recurrently flags "use the DS `loading` prop / remove the overlay" here; decline it when the no-jank behavior is intentional. (ENG-6395.)
...
- **Be skeptical of `void` — it's usually unnecessary.** `void`-ing a promise in a `() => void` handler is often cargo-culted; a block-body statement (`onReset={() => { setAllFilters(empty); }}`) lints clean without it. Don't add `void` reflexively, and flag it when you see it added — reach for it only when the linter genuinely requires marking an intentional floating promise (confirm by removing it and re-linting), and prefer `await` when completion/ordering matters. Default to dropping it. (BuiltByWalsh, #9679.)
