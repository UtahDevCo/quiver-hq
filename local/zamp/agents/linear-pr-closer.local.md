...
4. Upload screenshot evidence when available.
   - If the session has browser DevTools access and screenshot files exist in `scratch/linear/` for this ticket, attach them to the PR so reviewers can see visual evidence without checking out the branch.
   - **Use native `gh --attach`** (GitHub CLI ≥ v2.99.0) — it uploads media directly using `gh` CLI auth, no extension and no browser cookie. Do not install or use the old `gh-attach` extension (`atani/gh-attach`, `sudosubin/gh-attach`) or the release-tag hack. See the Screenshots & Attachments section of the Git Conventions for full details.
   - **Upload workflow**:
     1. Never copy screenshots into the repo and never create a `.screenshots/` directory as a fallback.
     2. Attach the screenshots directly when posting the PR comment (the `--attach` flag is repeatable; append `#alt text` for alt text):
        ```bash
        gh pr comment <PR_NUMBER> --body "## Manual Validation Screenshots

        **Before:** ![before](scratch/linear/<date>/<ISSUE-ID>-before.png)

        **After:** ![after](scratch/linear/<date>/<ISSUE-ID>-after.png)" \
          --attach scratch/linear/<date>/<ISSUE-ID>-before.png \
          --attach scratch/linear/<date>/<ISSUE-ID>-after.png
        ```
        The local `![alt](…)` paths in the body are rewritten in place to the uploaded assets.
   - If `gh --version` is below 2.99.0, the `--attach` flag is silently absent — bump `gh` (pinned in the nix flake) before relying on it.
